...
Amun::Amun(bool simulatorOnly, QObject *parent)
    : QObject(parent), m_processor(nullptr), m_transceiver(nullptr),
      m_networkTransceiver(nullptr), m_simulator(nullptr), m_referee(nullptr),
      m_vision(nullptr), m_mixedTeam(nullptr), m_simulatorEnabled(false),
      m_scaling(1.0f), m_useNetworkTransceiver(false),
      m_simulatorOnly(simulatorOnly), m_useInternalReferee(true),
      m_useAutoref(true), m_networkInterfaceWatcher(nullptr),
      m_seshat(new Seshat(20, this)), m_commandConverter(nullptr),
  ...
  qRegisterMetaType<std::shared_ptr<gameController::AutoRefToController>>(
      "std::shared_ptr<gameController::AutoRefToController>");
  ...
  qRegisterMetaType<camun::simulator::ErrorSource>(
      "camun::simulator::ErrorSource");
  ...
  connect(m_commandConverter, &CommandConverter::sendStatus, this,
          &Amun::handleStatus);
  ...
  m_networkInterfaceWatcher =
      (!m_simulatorOnly) ? new NetworkInterfaceWatcher(this) : nullptr;

  m_pathInputSaver[0].reset(new ProtobufFileSaver(
      "pathinput-blue.pathlog", "KHONSU PATHFINDING LOG", this));
  m_pathInputSaver[1].reset(new ProtobufFileSaver(
      "pathinput-yellow.pathlog", "KHONSU PATHFINDING LOG", this));
...
void Amun::start() {
  ...
  connect(m_processorThread, SIGNAL(finished()), m_processor,
          SLOT(deleteLater()));

  // route commands and replay status to processor and integrator
  connect(this, SIGNAL(gotCommand(Command)), m_processor,
  ...
  connect(this, SIGNAL(gotCommand(Command)), m_optionsManager,
          SLOT(handleCommand(Command)));
  connect(m_optionsManager, SIGNAL(sendStatus(Status)),
          SLOT(handleStatus(Status)));

  m_gitInfoRecorder = new GitInfoRecorder;
  m_gitInfoRecorder->moveToThread(m_gitRecorderThread);
  connect(m_gitRecorderThread, SIGNAL(finished()), m_gitInfoRecorder,
          SLOT(deleteLater()));
  connect(m_gitInfoRecorder, &GitInfoRecorder::sendStatus, this,
          &Amun::handleStatus);
  {
    QObject signalSource;
    connect(&signalSource, &QObject::destroyed, m_gitInfoRecorder,
            &GitInfoRecorder::recordRaGitDiff);
  }

  connect(this, SIGNAL(gotCommand(Command)), m_seshat,
          SLOT(handleCommand(Command)));
  connect(m_seshat, &Seshat::sendUi, this, &Amun::sendStatus);
  connect(m_seshat, &Seshat::sendReplayStrategy, this,
          &Amun::handleStatusForReplay);
  connect(m_seshat, &Seshat::simPauseCommand, this,
          &Amun::handleCommandLocally);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    connect(this, SIGNAL(gotCommand(Command)), m_debugHelper[i],
            SLOT(handleCommand(Command)));
    connect(m_debugHelper[i], SIGNAL(sendStatus(Status)),
            SLOT(handleStatus(Status)));
    connect(m_debugHelperThread, SIGNAL(finished()), m_debugHelper[i],
            SLOT(deleteLater()));

    m_gameControllerConnection[i].reset(new StrategyGameControllerMediator(
        m_processor->getInternalGameController(), i == 2));
    m_gameControllerConnection[i]->moveToThread(m_strategyThread[i]);
    connect(m_processor, &Processor::refereeHostChanged,
            m_gameControllerConnection[i].get(),
            &StrategyGameControllerMediator::handleRefereeHost);
    connect(this, &Amun::useInternalGameController,
            m_gameControllerConnection[i].get(),
            &StrategyGameControllerMediator::switchInternalGameController);
    connect(this, &Amun::useInternalGameController,
            m_processor->getInternalGameController(),
            &InternalGameController::setEnabled);
    connect(this, &Amun::gotCommandForGC,
            m_processor->getInternalGameController(),
            &InternalGameController::handleCommand);

    Q_ASSERT(m_strategy[i] == nullptr);
    ProtobufFileSaver *pathInput = m_pathInputSaver[std::min(1, i)].get();
    m_strategy[i] =
        new Strategy(m_timer, strategy, m_debugHelper[i], &m_compilerRegistry,
                     m_gameControllerConnection[i], i == 2, false, pathInput);
    m_strategy[i]->moveToThread(m_strategyThread[i]);
    connect(m_strategyThread[i], SIGNAL(finished()), m_strategy[i],
            SLOT(deleteLater()));

    // send tracking, geometry and referee to strategy
    connect(m_processor, SIGNAL(sendStrategyStatus(Status)), m_strategy[i],
            SLOT(handleStatus(Status)));
    connect(m_optionsManager, SIGNAL(sendStatus(Status)), m_strategy[i],
            SLOT(handleStatus(Status)));
    ...
    connect(
        ...
        m_processor,
        SLOT(handleStrategyCommands(bool, QList<RobotCommandInfo>, qint64)));
    connect(m_strategy[i], SIGNAL(sendHalt(bool)), m_processor,
            SLOT(handleStrategyHalt(bool)));
    connect(m_processor, SIGNAL(setFlipped(bool)), m_strategy[i],
            SLOT(setFlipped(bool)));

    // route commands from and to strategy
    connect(m_strategy[i], SIGNAL(gotCommand(Command)),
            SLOT(handleCommand(Command)));
    connect(this, SIGNAL(gotCommand(Command)), m_strategy[i],
            SLOT(handleCommand(Command)));
    // relay status and debug information of strategy
    connect(m_strategy[i], SIGNAL(sendStatus(Status)),
            SLOT(handleStatus(Status)));
    connect(m_strategy[i], SIGNAL(sendStatus(Status)), m_optionsManager,
            SLOT(handleStatus(Status)));
    connect(m_strategy[i], &Strategy::recordGitDiff, m_gitInfoRecorder,
            &GitInfoRecorder::startGitDiffStrategy);
  }

  // replay strategies and connections
  m_integrator = new Integrator();
  m_integrator->moveToThread(m_processorThread);
  connect(this, SIGNAL(gotCommand(Command)), m_integrator,
          SLOT(handleCommand(Command)));
  connect(this, SIGNAL(sendStatusForReplay(Status)), m_integrator,
          SLOT(handleReplayStatus(Status)));
  connect(m_processorThread, SIGNAL(finished()), m_integrator,
          SLOT(deleteLater()));
  // relay tracking, geometry, referee, controller and accelerator information
  connect(m_processor, SIGNAL(sendStrategyStatus(Status)), m_integrator,
          SLOT(handleStatus(Status)));
  for (int i = 0; i < 2; i++) {
    StrategyType strategy = i == 0 ? StrategyType::BLUE : StrategyType::YELLOW;
    Q_ASSERT(m_replayStrategy[i] == nullptr);
    m_replayStrategy[i] =
        new Strategy(m_replayTimer, strategy, nullptr, &m_compilerRegistry,
                     m_gameControllerConnection[i], false, true);
    // re-use thread for regular and replay strategy
    m_replayStrategy[i]->moveToThread(m_strategyThread[i + 3]);
    connect(m_strategyThread[i + 3], SIGNAL(finished()), m_replayStrategy[i],
            SLOT(deleteLater()));

    // use a rather large queue to make replay faster
    m_strategyBlocker[i] = new BlockingStrategyReplay(m_replayStrategy[i], 20);

    connect(m_integrator, SIGNAL(sendReplayStatus(Status)),
            m_strategyBlocker[i], SLOT(handleStatus(Status)));
    connect(m_optionsManager, SIGNAL(sendStatus(Status)), m_strategyBlocker[i],
            SLOT(handleStatus(Status)));

    connect(this, SIGNAL(gotCommand(Command)), m_replayStrategy[i],
            SLOT(handleCommand(Command)));
    connect(m_replayStrategy[i], SIGNAL(sendStatus(Status)),
            SLOT(handleReplayStatus(Status)));
    connect(m_replayStrategy[i], SIGNAL(sendStatus(Status)), m_optionsManager,
            SLOT(handleStatus(Status)));

    connect(m_seshat, &Seshat::changeStatusSource, m_replayStrategy[i],
            &Strategy::resetIsReplay);
  }

  if (!m_simulatorOnly) {
    // create referee
    setupReceiver(m_referee, QHostAddress(SSL_GAME_CONTROLLER_ADDRESS),
    ...
    connect(m_referee, &Receiver::gotPacket, m_processor,
            &Processor::handleRefereePacket);

    // create vision
    setupReceiver(m_vision, QHostAddress(SSL_VISION_ADDRESS),
                  SSL_SIMULATED_VISION_PORT);
    ...
    connect(m_mixedTeam, SIGNAL(gotPacket(QByteArray, qint64, QString)),
            m_processor, SLOT(handleMixedTeamInfo(QByteArray, qint64)));
  }
  ...
  connect(m_processor, SIGNAL(setFlipped(bool)), m_simulator,
          SLOT(setFlipped(bool)));

  connect(m_processor->getInternalGameController(),
          &InternalGameController::sendCommand, this, &Amun::handleCommand);

  if (!m_simulatorOnly) {
    Q_ASSERT(m_transceiver == nullptr);
    m_transceiver = new Transceiver(m_timer);
    m_transceiver->moveToThread(m_transceiverThread);
    connect(m_transceiverThread, SIGNAL(finished()), m_transceiver,
            SLOT(deleteLater()));
    // route commands to transceiver
    connect(this, SIGNAL(gotCommand(Command)), m_transceiver,
            SLOT(handleCommand(Command)));
    // relay transceiver status and timing
    connect(m_transceiver, SIGNAL(sendStatus(Status)),
            SLOT(handleStatus(Status)));
    ...
    connect(m_transceiverThread, SIGNAL(finished()), m_networkTransceiver,
            SLOT(deleteLater()));
    // route commands to transceiver
    connect(this, SIGNAL(gotCommand(Command)), m_networkTransceiver,
            SLOT(handleCommand(Command)));
    // relay transceiver status and timing
    connect(m_networkTransceiver, SIGNAL(sendStatus(Status)),
            SLOT(handleStatus(Status)));
  }

  connect(this, &Amun::gotCommand, m_commandConverter,
          &CommandConverter::handleCommand);
...
void Amun::stop() {
  ...
  // As the strategy may still be writing to debugHelper, we can only start
  // quitting as soon as the starategy is dead for sure.
...
void Amun::setupReceiver(Receiver *&receiver, const QHostAddress &address,
                         quint16 port) {
  ...
  // pass packets to processor
  connect(m_networkInterfaceWatcher, &NetworkInterfaceWatcher::interfaceUpdated,
          receiver, &Receiver::updateInterface);
}
...
void Amun::createSimulator(const amun::SimulatorSetup &setup) {
  ...
  connect(m_simulatorThread, SIGNAL(finished()), m_simulator,
          SLOT(deleteLater()));
  // pass on simulator and team settings
  connect(this, SIGNAL(gotCommand(Command)), m_simulator,
          SLOT(handleCommand(Command)));
...
void Amun::handleCommandLocally(const Command &command) {
  ...
  if (command->has_transceiver()) {
    if (command->transceiver().has_use_network()) {
      setSimulatorEnabled(m_simulatorEnabled,
                          command->transceiver().use_network());
    }
  }
  ...
  if (command->has_referee()) {
    ...
    // send out even if it does not change, since the default value may vary
    // between objects (GC)
...
void Amun::enableTrackingReplay() {
  ...
  connect(m_trackingReplay.get(), &TrackingReplay::gotStatus, this,
          &Amun::handleReplayStatus);
}
 ...
 * \ref Simulator and output of \ref Processor to either \ref Transceiver or
 * \ref Simulator.
 *
 * \param enabled Whether to enable or disable the simulator
 * \param useNetworkTransceiver Whether to send commands over network or using
 * the transceiver
 */
void Amun::setSimulatorEnabled(bool enabled, bool useNetworkTransceiver) {
  ...
  if (enabled || useNetworkTransceiver) {
    connect(m_processor, &Processor::sendRadioCommands, m_commandConverter,
            &CommandConverter::handleRadioCommands);
    if (enabled) {
      // simulator setup
      connect(m_commandConverter, &CommandConverter::sendSSLSim, m_simulator,
              &Simulator::handleRadioCommands);
    } else {
      // network transceiver setup
      connect(m_commandConverter, &CommandConverter::sendSSLSim,
              m_networkTransceiver, &NetworkTransceiver::handleSSLSimCommand);
      m_networkTransceiver->setSendCommands(true);
    }
  } else {
    // field setup
    connect(m_processor,
            SIGNAL(sendRadioCommands(QList<robot::RadioCommand>, qint64)),
            m_transceiver,
            SLOT(handleRadioCommands(QList<robot::RadioCommand>, qint64)));
  }

  // setup connection for simulator errors
  if (enabled) {
    connect(m_simulator, &Simulator::sendSSLSimError, m_commandConverter,
            &CommandConverter::handleSimulatorErrors);
  } else if (useNetworkTransceiver) {
    connect(m_networkTransceiver, &NetworkTransceiver::sendSSLSimError,
            m_commandConverter, &CommandConverter::handleSimulatorErrors);
  }
  ...
  if (enabled) {
    ...
    connect(m_simulator, &Simulator::sendRealData, m_processor,
            &Processor::handleSimulatorExtraVision);
  ...
  if (enabled) {
    connect(
        m_simulator, SIGNAL(sendRadioResponses(QList<robot::RadioResponse>)),
    ...
    connect(
        transceiver, SIGNAL(sendRadioResponses(QList<robot::RadioResponse>)),
        m_processor, SLOT(handleRadioResponses(QList<robot::RadioResponse>)));
  }
}
