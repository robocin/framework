...
const uint DEFAULT_VISION_PORT = SSL_SIMULATED_VISION_PORT;
...
const FieldWidgetAction DEFAULT_ROBOT_DOUBLE_CLICK_ACTION =
    FieldWidgetAction::ToggleVisualization;
...
const FieldWidgetAction DEFAULT_ROBOT_CTRL_CLICK_ACTION =
    FieldWidgetAction::SetDebugSearch;
...
static QPalette createQPalette(const QColor foreground, const QColor background,
                               const QColor alternateBackground,
                               const QColor disabled, const QColor highlight,
                               const QColor link) {
  QPalette palette;
  ...
  const auto base =
      isDarkMode ? background.darker(150) : background.lighter(150);
...
static const QStringList COLOR_SCHEME_ITEMS = {"Default", "Light", "Dark",
                                               "Custom"};
...
ConfigDialog::ConfigDialog(QWidget *parent)
  ...
  for (FieldWidgetAction action :
       {FieldWidgetAction::None, FieldWidgetAction::SetDebugSearch,
        FieldWidgetAction::ToggleVisualization}) {
    ui->doubleClickAction->addItem(robotActionString(action),
                                   static_cast<int>(action));
    ui->ctrlClickAction->addItem(robotActionString(action),
                                 static_cast<int>(action));
  }

  connect(ui->colorScheme, SIGNAL(currentIndexChanged(int)), this,
          SLOT(changedPalette(int)));
...
void ConfigDialog::sendConfiguration() {
  ...
  amun::TransceiverConfiguration *c =
      command->mutable_transceiver()->mutable_configuration();
  c->set_channel(ui->comboChannel->currentIndex());

  // from ms to ns
  command->mutable_tracking()->set_system_delay(ui->systemDelayBox->value() *
                                                1000 * 1000);
  ...
  amun::HostAddress *nc =
      command->mutable_transceiver()->mutable_network_configuration();
  nc->set_host(ui->networkHost->text().toStdString());
  nc->set_port(ui->networkPortSimulator->value());

  amun::SimulatorNetworking *sn =
      command->mutable_transceiver()->mutable_simulator_configuration();
  ...
  command->mutable_mixed_team_destination()->set_host(
      ui->mixedHost->text().toStdString());
  command->mutable_mixed_team_destination()->set_port(ui->mixedPort->value());

  emit sendCommand(command);

  emit useNumKeysForReferee(ui->numKeyUsage->currentText() !=
                            DEFAULT_NUMBER_KEYS_USAGE);

  emit setRobotDoubleClickAction(
      static_cast<FieldWidgetAction>(
          ui->doubleClickAction->currentData().toInt()),
      ui->doubleClickSearch->text());
  emit setRobotCtrlClickAction(static_cast<FieldWidgetAction>(
                                   ui->ctrlClickAction->currentData().toInt()),
  ...
  if (colorSchemeSetting == "Default") {
    ...
    palette = createQPalette(foreground, background, alternateBackground,
    ...
    // const auto alternateBackground =
    // ui->alternateBackgroundColor->getColor(); const auto disabled =
    // ui->disabledColor->getColor(); const auto highlight =
    // ui->highlightColor->getColor(); const auto link =
    // ui->linkColor->getColor();
    palette = createQPalette(customPalette.foreground, customPalette.background,
                             customPalette.alternateBackground,
                             customPalette.disabled, customPalette.highlight,
                             customPalette.link);
  }

  QApplication::setStyle(style);

  // for some reason you need both to the QApplication::setPalette and the
  // setPalette for every widget don't ask me why, it's Qt magic
...
void ConfigDialog::load() {
  QSettings s;
  ui->comboChannel->setCurrentIndex(
      s.value("Transceiver/Channel", DEFAULT_TRANSCEIVER_CHANNEL).toUInt());
  ui->systemDelayBox->setValue(
      s.value("Tracking/SystemDelay", DEFAULT_SYSTEM_DELAY).toUInt()); // in ms

  ui->visionPort->setValue(
      s.value("Amun/VisionPort2018", DEFAULT_VISION_PORT).toUInt());
  ui->refPort->setValue(
      s.value("Amun/RefereePort", DEFAULT_REFEREE_PORT).toUInt());

  ui->networkUse->setChecked(
      s.value("Network/Use", DEFAULT_NETWORK_ENABLE).toBool());
  ui->networkHost->setText(
      s.value("Network/Host", DEFAULT_NETWORK_HOST).toString());

  ui->controlSimulator->setChecked(
      s.value("Network/ControlSimulator", DEFAULT_CONTROL_SIMULATOR).toBool());
  ui->networkPortSimulator->setValue(
      s.value("Network/PortControl", DEFAULT_NETWORK_PORT).toUInt());

  ui->controlBlue->setChecked(
      s.value("Network/ControlBlue", DEFAULT_CONTROL_BLUE).toBool());
  ui->networkPortBlue->setValue(
      s.value("Network/PortBlue", DEFAULT_NETWORK_PORT_BLUE).toUInt());

  ui->controlYellow->setChecked(
      s.value("Network/ControlYellow", DEFAULT_CONTROL_YELLOW).toBool());
  ui->networkPortYellow->setValue(
      s.value("Network/PortYellow", DEFAULT_NETWORK_PORT_YELLOW).toUInt());
  ...
  ui->numKeyUsage->setCurrentText(
      s.value("Ui/NumKeyUsage", DEFAULT_NUMBER_KEYS_USAGE).toString());

  FieldWidgetAction robotDoubleClick = static_cast<FieldWidgetAction>(
      s.value("Ui/RobotDoubleClickAction",
              static_cast<int>(DEFAULT_ROBOT_DOUBLE_CLICK_ACTION))
          .toInt());
  ui->doubleClickAction->setCurrentText(robotActionString(robotDoubleClick));
  ui->doubleClickSearch->setText(
      s.value("Ui/RobotDoubleClickSearch",
              DEFAULT_ROBOT_DOUBLE_CLICK_SEARCH_STRING)
          .toString());

  FieldWidgetAction robotCtrlClick = static_cast<FieldWidgetAction>(
      s.value("Ui/RobotCtrlClickAction",
              static_cast<int>(DEFAULT_ROBOT_CTRL_CLICK_ACTION))
          .toInt());
  ui->ctrlClickAction->setCurrentText(robotActionString(robotCtrlClick));
  ui->ctrlClickSearch->setText(
      s.value("Ui/RobotCtrlClickSearch", DEFAULT_ROBOT_CTRL_CLICK_SEARCH_STRING)
          .toString());

  m_defaultVersionString =
      s.value("Config/ConfigDialogDefaultVersion", PREALL_CONFIG_DATE)
          .toString();
  if (m_defaultVersionString != CURRENT_CONFIG_DATE) {
    // Sanitize old double click search string
    if (ui->doubleClickSearch->text() ==
        DEFAULT_ROBOT_DOUBLE_CLICK_SEARCH_STRING_2020) {
      ui->doubleClickSearch->setText(DEFAULT_ROBOT_DOUBLE_CLICK_SEARCH_STRING);
    }
  }

  ui->scrollSensitivitySpinBox->setValue(
      s.value("Ui/ScrollSensitivity", DEFAULT_SCROLL_SENSITIVITY).toDouble());
...
void ConfigDialog::reset() {
  ...
  ui->doubleClickAction->setCurrentText(
      robotActionString(DEFAULT_ROBOT_DOUBLE_CLICK_ACTION));
  ui->doubleClickSearch->setText(DEFAULT_ROBOT_DOUBLE_CLICK_SEARCH_STRING);
  ui->ctrlClickAction->setCurrentText(
      robotActionString(DEFAULT_ROBOT_CTRL_CLICK_ACTION));
...
void ConfigDialog::apply() {
  ...
  s.setValue("Ui/RobotDoubleClickAction",
             ui->doubleClickAction->currentData().toInt());
  s.setValue("Ui/RobotDoubleClickSearch", ui->doubleClickSearch->text());
  s.setValue("Ui/RobotCtrlClickAction",
             ui->ctrlClickAction->currentData().toInt());
