 ...
 *
 * Known issues:
 *  - [ ]: Currently, it is not possible to supply partial positions for
 * teleportBall or teleportRobot
 *  - [ ]: Robots go into standby after 0.1 seconds without command (Safty)
 *  - [ ]: It is not possible to change specs or geometry without resetting the
 * world
 *  - [ ]: It is not possible to setUp a team with no robots (You can still
 * teleport them away)
 *  - [ ]: It is not possible to supply only partial specs. It is planned to
 * fuse them with the last specs for this team if not supplied, but NYI.
 *  - [ ]: Dribbler will reset if a new command doesn't contain a new dribbling
 * speed (contrary to the definition that states all not set values should stay
 * as previously assumed)
 *  - [ ]: Commands that are recieved at t0 will not be in effect after the next
 * tick of the simulator (around 5 ms), no interpolation.
...
public slots:
  void handleSimulatorError(const QList<SSLSimError> &error,
                            camun::simulator::ErrorSource source);
...
SimulatorCommandAdaptor::SimulatorCommandAdaptor(Timer *timer,
                                                 SSLVisionServer *vision)
    : m_server(this), m_senderAddress(QHostAddress::Null), m_senderPort(-1),
      m_timer(timer), m_visionServer(vision) {
  m_server.bind(QHostAddress::Any, SSL_SIMULATION_CONTROL_PORT);
  connect(&m_server, &QUdpSocket::readyRead, this,
          &SimulatorCommandAdaptor::handleDatagrams);
}
...
signals:
  void sendRadioCommands(const SSLSimRobotControl &commands, bool isBlue,
                         qint64 processingDelay);
...
RobotCommandAdaptor::RobotCommandAdaptor(bool blue, Timer *timer)
    : m_is_blue(blue), m_server(this), m_senderAddress(QHostAddress::Null),
      m_senderPort(-1), m_timer(timer) {
  m_server.bind(QHostAddress::Any, (blue) ? SSL_SIMULATION_CONTROL_BLUE_PORT
                                          : SSL_SIMULATION_CONTROL_YELLOW_PORT);
  connect(&m_server, &QUdpSocket::readyRead, this,
          &RobotCommandAdaptor::handleDatagrams);
}
...
static void setError(sslsim::SimulatorError *error, SimError code,
                     SimErrorSource source, std::string appendix = "") {
  ...
  case SimError::UNSUPPORTED_VELOCITY:
    ...
    message = "The received message had a velocity type unsupported by this "
              "simulator " +
              appendix;
    break;
  case SimError::UNSUPPORTED_ANGLE:
    codeStr = "ANGLE_VALUE";
    message =
        "The received kick angle was not equal to either 0 or 45 " + appendix;
    break;
  case SimError::MISSING_SPEC:
    codeStr = "INVALID_SPEC";
    message = "The received spec is missing one of the required fields for "
              "this simulator " +
              appendix;
    break;
  case SimError::INVALID_REALISM:
    codeStr = "INVALID_REALISM";
    message = "The received realism is not conforming to the realism "
...
static void sendUDP(const google::protobuf::Message &out, QUdpSocket &server,
                    const QHostAddress &senderAddress, int senderPort) {
  QByteArray data;
  data.resize(out.ByteSize());
  bool sendingSuccessful = false;
  if (out.SerializeToArray(data.data(), data.size())) {
    sendingSuccessful =
        server.writeDatagram(data, senderAddress, senderPort) == data.size();
  }
...
#define SCALE_UP(OBJ, ATTR)                                                    \
  do {                                                                         \
    if ((OBJ).has_##ATTR())                                                    \
      (OBJ).set_##ATTR((OBJ).ATTR() * 1e3);                                    \
  } while (0)

static void warnLatency(qint64 delta) {
  if (delta > 1e6) {
    log(stdout,
        "Warning: Handled Datagram in %lldns, should be lower than 1e6\n",
        delta);
  }
}

// TODO: Always update the following constant if the robotSpecs did change,
//  either disregard the new field and just increase the expected number if the
//  new field is useless to our simulator, or convert it properly and update
//  this number.
constexpr int expected_specs_fields = 10 + 3;
constexpr int functionToFixForSpecs = __LINE__;
template <class T>
static bool convertSpecsToErForce(
    T outGen, const sslsim::RobotSpecs &in) // @return false: Error occured
{
  if (!in.has_mass()) {
    return false;
  }
  ...
  if (in.has_max_chip_kick_speed()) {
    out->set_shot_chip_max(
        coordinates::chipDistanceFromChipVel(in.max_chip_kick_speed()));
  } else {
  ...
  out->set_dribbler_height(
      0.04 /*rsef.dribbler_height()*/); // FIXME: We use only our specs, because
                                        // we don't know if dribbling will be
                                        // possible at all with any other values
                                        // :/
...
void SimulatorCommandAdaptor::handleDatagrams() {
  while (m_server.hasPendingDatagrams()) {
    ...
    if (simcom.has_control()) {
      ...
      for (sslsim::TeleportRobot &robot :
           *sslControl->mutable_teleport_robot()) {
        SCALE_UP(robot, x);
        SCALE_UP(robot, y);
        SCALE_UP(robot, v_x);
        SCALE_UP(robot, v_y);
    ...
    if (simcom.has_config()) {
      ...
      if (config.has_geometry()) {
        ...
        convertFromSSlGeometry(config.geometry().field(),
                               *(setup->mutable_geometry()));
      ...
      if (config.robot_specs_size() > 0) {
        ...
        for (const auto &spec : config.robot_specs()) {
          bool success = convertSpecsToErForce(
              [&blueTeam, &yellowTeam, &c](bool isBlue) {
                if (isBlue) {
                  if (blueTeam == nullptr) {
          ...
          if (!success) {
            sendSir = true;
            setError(sir.add_errors(), SimError::MISSING_SPEC,
                     SimErrorSource::CONTROLLER, spec.DebugString());
            newSz--;
          }
        }
...
void RobotCommandAdaptor::handleSimulatorError(
    const QList<SSLSimError> &error, camun::simulator::ErrorSource source) {

  camun::simulator::ErrorSource expected =
      m_is_blue ? camun::simulator::ErrorSource::BLUE
                : camun::simulator::ErrorSource::YELLOW;
  if (source != expected)
    return;
  if (error.size() == 0)
    return;
...
void SimulatorCommandAdaptor::handleSimulatorError(
    const QList<SSLSimError> &error, camun::simulator::ErrorSource source) {
  if (source != camun::simulator::ErrorSource::CONFIG)
    return;
  if (error.size() == 0)
    return;
  sslsim::SimulatorResponse sir;
...
void RobotCommandAdaptor::handleDatagrams() {
  const SimErrorSource ERROR_SOURCE =
      m_is_blue ? SimErrorSource::BLUE_TEAM : SimErrorSource::YELLOW_TEAM;
  while (m_server.hasPendingDatagrams()) {
    qint64 start = m_timer->currentTime();
    sslsim::RobotControlResponse rcr;
    bool sendRcr = false;
      ...
      if (command.has_move_command()) {
        ...
        if (moveCmd.has_wheel_velocity() || moveCmd.has_global_velocity()) {
          ...
          const std::string robotStr =
              "(Robot :" + std::to_string(command.id()) + ")";
          setError(rcr.add_errors(), SimError::UNSUPPORTED_VELOCITY,
                   ERROR_SOURCE, robotStr);
        }
      }
    }
    emit sendRadioCommands(control, m_is_blue,
                           m_timer->currentTime()); // This might be a bit late.
    // TODO: response!
    qint64 delta = m_timer->currentTime() - start;
...
void RobotCommandAdaptor::handleRobotResponse(
    const QList<robot::RadioResponse> &res) {
  ...
  for (const auto &response : res) {
    if (response.has_is_blue() && response.is_blue() == m_is_blue &&
        response.has_ball_detected()) {
...
void RobotCommandAdaptor::sendRobotRespose(
    const sslsim::RobotControlResponse &out) {
...
void SSLVisionServer::setPort(int port) { m_server.change_port(port); }
...
void SimProxy::handleCommand(const Command &command) {
  bool hasSimSetup =
      command->has_simulator() && command->simulator().has_simulator_setup();
  ...
  if (command->has_set_team_yellow()) {
    m_teamCommand->mutable_set_team_yellow()->CopyFrom(
        command->set_team_yellow());
    if (hasSimSetup) {
      command->clear_set_team_yellow();
    }
  }
  if (command->has_simulator() && command->simulator().has_realism_config()) {
    m_teamCommand->mutable_simulator()->mutable_realism_config()->CopyFrom(
        command->simulator().realism_config());
    if (hasSimSetup) {
  ...
  if (hasSimSetup) {
    ...
    connect(this, &SimProxy::handleRadioCommands, m_sim,
            &Simulator::handleRadioCommands);
    connect(m_sim, &Simulator::sendSSLSimError, this,
            &SimProxy::sendSSLSimError);
    connect(m_sim, &Simulator::sendRadioResponses, this,
            &SimProxy::sendRadioResponses);
...
int main(int argc, char *argv[]) {
  ...
  QCommandLineOption geometryConfig({"g", "geometry"},
                                    "The geometry file to load as default",
                                    "file", "2020B");
  QCommandLineOption realismConfig(
      "realism",
      "Simulator realism configuration (short file name without the .txt)",
      "realism", "Realistic");
  ...
  if (real_fields != expected_specs_fields) {
    std::string msg = "BUG: The number of fields for the specs has a different "
                      "number compared to expected!";
  ...
  blue.connect(&blue, &RobotCommandAdaptor::sendRadioCommands, &sim,
               &SimProxy::handleRadioCommands);
  blue.connect(&sim, &SimProxy::sendRadioResponses, &blue,
               &RobotCommandAdaptor::handleRobotResponse);
  yellow.connect(&yellow, &RobotCommandAdaptor::sendRadioCommands, &sim,
                 &SimProxy::handleRadioCommands);
  yellow.connect(&sim, &SimProxy::sendRadioResponses, &yellow,
                 &RobotCommandAdaptor::handleRobotResponse);

  vision.connect(&sim, &SimProxy::gotPacket, &vision,
                 &SSLVisionServer::sendVisionData);
  commands.connect(&commands, &SimulatorCommandAdaptor::sendCommand, &sim,
                   &SimProxy::handleCommand);

  commands.connect(&sim, &SimProxy::sendSSLSimError, &commands,
                   &SimulatorCommandAdaptor::handleSimulatorError);
  blue.connect(&sim, &SimProxy::sendSSLSimError, &blue,
               &RobotCommandAdaptor::handleSimulatorError);
  yellow.connect(&sim, &SimProxy::sendSSLSimError, &yellow,
                 &RobotCommandAdaptor::handleSimulatorError);
  ...
  if (!loadConfiguration("simulator/" + parser.value(geometryConfig),
                         c->mutable_simulator()->mutable_simulator_setup(),
                         false)) {
    exit(EXIT_FAILURE);
  }
  if (!loadConfiguration("simulator-realism/" + parser.value(realismConfig),
                         c->mutable_simulator()->mutable_realism_config(),
                         true)) {
    exit(EXIT_FAILURE);
  }
